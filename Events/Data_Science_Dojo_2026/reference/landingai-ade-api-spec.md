...
### 5. Build Extract Schema

**Endpoint**: `POST /v1/ade/extract/build-schema`
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `document` | file | One required | Markdown files or inline content strings to analyze for schema generation. Multiple documents can be provided for better schema coverage. |
| `markdown_urls` | string | One required | URLs to Markdown files to analyze for schema generation. |
| `model` | string | No | The version of the model to use for schema generation (default: `extract-latest`) |
| `prompt` | string | No | Instructions for how to generate or modify the schema. |
| `schema` | string | No | Existing JSON schema to iterate on or refine. |

#### Response Structure

```
.extraction_schema         → string: The generated JSON schema as a string.
.metadata          → {filename, org_id, duration_ms, credit_usage, version, job_id, warnings}
```
