"""
Utility-bill processing pipeline using the LandingAI ADE **v2** (DPT-3) async APIs.
...
Steps:
  ...
  3. Load the extraction schema from the shared ../utility_bill.json (used by v1 too).
...
Only v2 endpoints are used. Run from anywhere:
    ...
    python process_utility_bills.py
"""
...
INPUT_FOLDER = HERE.parent / "input_folder"          # Utility_Bills/input_folder
SCHEMA_PATH = HERE.parent / "utility_bill.json"      # shared schema, reused by v1 and v2
...
ENV_PATH = HERE.parent / ".env"                      # Utility_Bills/.env
...
def build_summaries(
    ...
    _save_csv(fields_rows, CSV_DIR / "utility_bill_fields_summary.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Utility-bill pipeline on ADE v2 (DPT-3).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Process only these documents by name stem (e.g. --only electric1 electric_A).",
    )
