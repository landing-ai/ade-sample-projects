"""
Invoice processing pipeline using the LandingAI ADE **v2** (DPT-3) async APIs.

Steps (per the Invoice Sample Project Instructions):
  1. Submit every file in input_folder to Parse Jobs v2 (standard service_tier).
  2. Save each parse result as <name>.json and its Markdown as <name>.md
     under results_folder/parse/.
  3. Load the extraction schema from schema/invoice_demo_schema.json.
  4. Submit each parsed document's Markdown + schema to Extract Jobs v2
     (standard service_tier).
  5. Save each extract result as <name>.json under results_folder/extract/.
  6. Write a CSV of all extracted fields except line_items (one row per document),
     plus job_id, document name, and processing date.
  7. Write a CSV of all line_item details (one row per line item), keyed by job_id,
     plus document name and processing date.
  8. Write a CSV of parse + extract metadata (one row per document).
...
INPUT_FOLDER = HERE.parent / "input_folder"          # Use_Cases/Invoices/input_folder
SCHEMA_PATH = HERE / "schema" / "invoice_demo_schema.json"
...
ENV_PATH = HERE.parent / ".env"                      # Use_Cases/Invoices/.env

SERVICE_TIER = "standard"
SUPPORTED_SUFFIXES = {".pdf", ".png", ".jpg", ".jpeg", ".tif", ".tiff"}

# Fields that live at the top level of the extraction but are handled separately
# (line_items goes to its own CSV, not the flat per-document CSV).
...
def flatten(obj: Any, prefix: str = "", sep: str = ".") -> dict[str, Any]:
    ...
    (the only list we care about, line_items, is excluded before flattening).
    """
    flat: dict[str, Any] = {}
...
def discover_documents(folder: Path) -> list[Path]:
    """Return supported document files in folder, sorted by natural invoice order."""
    docs = [p for p in folder.iterdir() if p.suffix.lower() in SUPPORTED_SUFFIXES]

    def sort_key(p: Path) -> tuple[str, int]:
        # invoice_1, invoice_2, ... invoice_10 in numeric order when possible
        stem = p.stem
...
def build_summaries(
    ...
    """Steps 6-8: write the three summary CSVs."""
    ...
    for stem, result in extracted.items():
        ...
        # --- Step 6: flat per-document fields (everything except line_items) ---
        fields_only = {k: v for k, v in extraction.items() if k != LINE_ITEMS_KEY}
        row = {
            "document_name": stem,
            "job_id": extract_job_id,
            "processing_date": processing_date,
        }
        row.update(flatten(fields_only))
        fields_rows.append(row)

        # --- Step 7: line items (one row each), keyed by job_id ---
        for idx, item in enumerate(extraction.get(LINE_ITEMS_KEY) or []):
            li_row = {
                "job_id": extract_job_id,
                "document_name": stem,
                "processing_date": processing_date,
            ...
            if isinstance(item, dict):
                li_row.update(flatten(item))
            line_item_rows.append(li_row)

        # --- Step 8: parse + extract metadata, one row per document ---
    ...
    _save_csv(fields_rows, CSV_DIR / "invoice_fields_summary.csv")
    _save_csv(line_item_rows, CSV_DIR / "invoice_line_items.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Invoice pipeline on ADE v2 (DPT-3).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Process only these documents by name stem (e.g. --only invoice_2 invoice_3).",
    )
