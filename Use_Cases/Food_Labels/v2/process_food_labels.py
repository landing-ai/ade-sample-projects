"""
Food-label processing pipeline using the LandingAI ADE **v2** (DPT-3) async APIs.
...
Steps:
  ...
  3. Load the extraction schema from schema/food_label_schema.json.
...
INPUT_FOLDER = HERE.parent / "input_folder"          # Food_Labels/input_folder
SCHEMA_PATH = HERE / "schema" / "food_label_schema.json"
...
ENV_PATH = HERE.parent / ".env"                      # Food_Labels/.env
...
def build_summaries(
    ...
    _save_csv(fields_rows, CSV_DIR / "food_label_fields_summary.csv")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Food-label pipeline on ADE v2 (DPT-3).")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Process only these documents by name stem (e.g. --only chomps goji).",
    )
